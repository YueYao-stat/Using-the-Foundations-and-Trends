...
Anahita Gupta \\
now publishers, Inc. \\
anahita.gupta@nowpublishers.com
\and
Mike Casey \\
now publishers, Inc. \\
...
\author[1]{Gupta,Anahita}
\author[2]{Casey,Mike}

\affil[1]{now publishers, Inc.; anahita.gupta@nowpublishers.com}
...
\chapter{Implementing Accessibility}

In 2025, the European Union are implementing new laws with regards to making online publications more accessible to visually impaired readers. Other geographic regions are likely to follow suit in the short-term. We are working to implement these rules as soon as possible. To comply, please adgere to the  requirements laid out in this chapter when coding your source.

The LaTeX team are working on a tagging project to easily implement the requirements for accessible PDFs. Full details with tips, can be found at: \url{https://github.com/latex3/tagging-project}.

\section{Preamble}
Insert the following code in the preamble before \verb+\documentclass+.
...
\section{Tables}
The headers of tables need to be identified. Use the command: \\
\verb+\tagpdfsetup{table/header-rows=1}+\\ 
to indicate how many rows are headers at the start ofthe tabular environment. For example:
\begin{small}
\begin{verbatim}
\begin{table}[h]
\begin{tabular}{lcc}
\tagpdfsetup{table/header-rows=1}
{\textbf Journal} & {\textbf <jrnlcode> } & {\textbf Ref. 
	Style }\\
\hline
...
If your table does not contain headers and is being used only for presentation use: \verb+\tagpdfsetup{table/tagging=presentation}+

%More information on accessibility in tables can be found at 
See \url{https://ctan.math.illinois.edu/macros/latex/required/latex-lab/latex-lab-table.pdf} for full details on accessibilty tables.

\section{Figures}
In addition to a caption, each figure needs a text that describes its content to a visually impaired reader. For example, an Alt Text for the now publishers logo could be: {\it The now logo, comprising the word ``now'' in the bimini font with the inner circle of the letter ``O'' coloured green.} To implement this add the {\it alt= option} to \verb+\includegraphics+ (see below).

\begin{verbatim}
\begin{figure}[ht!]
\centering
\includegraphics[width=0.6\textwidth,alt=The now logo, 
comprising the word ``now'' in the bimini font with the 
inner circle of the letter ``O'' coloured green.]
{now_logo.pdf}
\caption{Now Publishers logo.}
\end{figure}
\end{verbatim}
...
\section{Installation}
To check your installation, try compiling this article. You will find it in: \verb+nowfnt\SampleArticle+ in the distribution. Make sure you either have  copies of the class and style files in the same folder as the source, or that your TeXMF path is set up correctly. If you cannot compile this file, please consult your system manager for assistance.

\section{Compilation Errors}
If you get errors during the compilation of your manuscript, please send an email to latexsupport@nowpublishers.com describing the error and the message that you receive. Include in the email the full \LaTeX\ source and the .log file created by the compiler. We will be happy to assist you further.
...
\small
FnT Cognitive Science & COG & Author/Year \\
\small
FnT Comm. and Information Theory & CIT & Numeric \\
\small
FnT Computer Vision and Graphics & CGV & Numeric \\
\small
...
\small
FnT Integrated Circuits and Systems & ICS & Numeric \\
\small
...
\small
FnT Optics and Photonics & OAP & Numeric \\
\small
...
\small
FnT Statistics & STA & Author/Year \\
\small
